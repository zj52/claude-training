...
This is the third and final step in the *Theory → Policy → Operational Tooling* arc. Module 3 is where the abstract becomes concrete:

- **Module 1 (Foundations)** — *the tour.* You met the tool and learned the data rules.
- **Module 2 (AI Fluency)** — *the thinking.* You learned the framework and drafted your unit's AI policy.
- **Module 3 (Cowork)** — *the operational artifact.* You configure Cowork with your policy. The policy becomes Global Instructions. Your scope becomes a Project. The folder you select becomes the persistent context. The "AI can handle" tasks from your policy become Skills.
...
Draft order — refine after Module 2 has been delivered and the cohort's policy drafts and reflections are in.
...
7. **From your unit-level policy to Global Instructions + Project.** This is the bridge slide. Explicit map: policy → Global Instructions (role, tone, formatting rules) + Project init scope (data class, values, allowed tasks). The Module 2 deliverable is now the configuration.
...
14. **Hands-on — Configure → Build → Run live.** *(The centerpiece, ~25 min.)* Three-part live build:
    - **(a) Configure** Global Instructions from the participant's policy draft + start a Project pointed at the chosen folder.
    - **(b) Build** a first Skill from one of the policy's "AI can handle" tasks via the skill-creator interview.
    - **(c) Run** the task end-to-end with the new skill active. Watch the Progress panel.

    The room sees the **Plan → Execute → Connect** loop happen on real Cornell work, with their policy and a new skill *both* driving the behavior. Builds the habit of *"my policy isn't a doc — it's how the tool now behaves."*
...
- **Pre-work:** in the email going out before Module 3, ask participants to bring (1) their unit-level AI policy draft from Module 2, (2) one specific folder they want Cowork to work in (a sponsor folder, a recurring report folder, an award folder), and (3) one specific task in their queue that they'd like to delegate — *picked from the "AI can handle" section of their policy.*
- **Live exercise (~25 min):**

  **(a) Configure (~7 min)** — Global Instructions + Project
  1. Open Claude Desktop → switch to Cowork mode.
  2. Settings → Cowork → paste a condensed version of the unit-level policy as **Global Instructions**.
...
- **Debrief (~10 min):**
  ...
  - What in the policy needed to be sharper to make this run cleanly?
  - **The takeaway to land:** *Your policy isn't a doc you wrote — it's how the tool now behaves. The scaffold you just made isn't yours to polish alone — bring it to RAIS / the AI Innovation Hub to refine, and it can become part of the cohort's shared skill library.*
...
- `MODULE-2-PLAN.md` — the policy → Cowork bridge originates here
...
- **Yellow data → use Cowork with the unit-level policy as Global Instructions** (internal proposal drafts, sponsor negotiation notes, unit budget drafts).
...
1. **What did Module 2's cohort actually deliver?** Wait until policy drafts are in before designing examples — let the actual policies shape what the live exercise demonstrates.
